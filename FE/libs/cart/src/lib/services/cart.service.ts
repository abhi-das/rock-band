...
export const CartModuleConfigToken = new InjectionToken<CartModuleConfig>('Cart Module Config Token');
...
export class CartService {
	...
	loadProductInCart(): Observable<cartModel.CartProductEntry[]> {
		return this.http.get<cartModel.CartProductEntry[]>(this.cartServiceConfig.apiURL).pipe(
			map((res) => res)
			// map((res) => {
